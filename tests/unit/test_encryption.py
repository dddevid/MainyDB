...
class TestSHA256Hasher:
    ...
    def test_verify_correct_password(self):
        ...
        assert hasher.verify("test_password", stored_hash) == True
    ...
    def test_same_password_different_hashes(self):
        ...
        assert hasher.verify("test_password", hash1) == True
        assert hasher.verify("test_password", hash2) == True
...
class TestAES256Cipher:
    ...
    def test_different_keys_produce_different_results(self):
        ...
        # Ciphers with different keys should produce different results
        # (even though we can't directly compare due to IV differences)
...
class TestEncryptionConfig:
    ...
    def test_add_sha256_field(self):
        ...
        assert config.is_sha256_field("password") == True
        assert config.is_aes256_field("password") == False
        assert config.is_encrypted_field("password") == True
    ...
    def test_add_aes256_field(self):
        ...
        assert config.is_aes256_field("email") == True
        assert config.is_sha256_field("email") == False
        assert config.is_encrypted_field("email") == True
    ...
    def test_create_encryption_config_helper(self):
        ...
        assert config.is_sha256_field("password") == True
        assert config.is_aes256_field("email") == True
        assert config.is_aes256_field("ssn") == True
...
class TestDatabaseIntegration:
    ...
    def test_insert_with_aes256_encryption(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Find the user (should decrypt automatically)
            found_user = users.find_one({"username": "john"})
    ...
    def test_update_with_encryption(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Email should be decrypted to new value
            assert found_user["email"] == "new@example.com"
            # Password should be hashed
            assert isinstance(found_user["password"], dict)


# Need to import Database for integration tests
